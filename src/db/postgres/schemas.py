...
class User(Base):
    ...
    last_login_at: Mapped[datetime | None] = mapped_column(pg.TIMESTAMP(timezone=True))
...
class AppIntegrationsCredentials(Base):
    ...
    user_id: Mapped[uuid.UUID] = mapped_column(
        pg.UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False
    )
    ...
    token_type: Mapped[str] = mapped_column(pg.VARCHAR(20), default="Bearer", nullable=False)

    scopes: Mapped[list] = mapped_column(pg.ARRAY(String), nullable=False)
    access_token_expiry: Mapped[datetime | None] = mapped_column(pg.TIMESTAMP(timezone=True), nullable=True)
    refresh_token_expiry: Mapped[datetime | None] = mapped_column(pg.TIMESTAMP(timezone=True), nullable=True)
...
class OAuthAccounts(Base):
    ...
    user_id: Mapped[uuid.UUID] = mapped_column(
        pg.UUID(as_uuid=True), ForeignKey("users.id"), nullable=False
    )
    integration_id: Mapped[uuid.UUID] = mapped_column(
        pg.UUID(as_uuid=True), ForeignKey("app_integrations_credentials.id", ondelete="SET NULL")
    )
    ...
    __table_args__ = (
        UniqueConstraint("provider", "provider_sub", name="uq_provider_sub"),  # composite unique
    )
