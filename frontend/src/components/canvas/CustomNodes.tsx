...
const getNodeIcon = (key: string, size = 18) => {
  if (key.startsWith('llm.')) return <Brain className="text-amber-400" size={size} />;
  if (key.startsWith('gmail.')) return <Mail className="text-rose-400" size={size} />;
  if (key.startsWith('sheets.')) return <FileSpreadsheet className="text-emerald-400" size={size} />;
  if (key.startsWith('drive.')) return <FolderUp className="text-sky-400" size={size} />;
  if (key === 'if_node' || key === 'switch_node') return <GitFork className="text-purple-400" size={size} />;
  ...
  switch (type) {
    case 'LLM':
      return {
        ...
        borderColorSelected: 'border-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.4)]',
    ...
    case 'ACTION':
      return {
        ...
        borderColorSelected: 'border-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.4)]',
    ...
    case 'CONTROL_FLOW':
      return {
        ...
        borderColorSelected: 'border-purple-500 shadow-[0_0_15px_rgba(168,85,247,0.4)]',
    ...
    default:
      return {
        ...
        borderColorSelected: 'border-slate-500 shadow-[0_0_15px_rgba(100,116,139,0.4)]',
...
export const WFlowCustomNode: React.FC<CustomNodeProps> = ({ data, selected }) => {
  const spec = NODE_SPEC_CATALOG[data.key];
  ...
  return (
    <div 
      ...
      style={{
        ...
        boxShadow: selected ? undefined : '0 10px 25px -5px rgba(0, 0, 0, 0.5), 0 8px 10px -6px rgba(0, 0, 0, 0.5)'
      }}
    >
        ...
        {data.key === 'if_node' && (
          <div className="bg-slate-900/40 p-1.5 rounded border border-slate-800/40 font-mono text-[9px] space-y-1">
            <div className="flex items-center justify-between">
              ...
              <span className="text-purple-400 truncate max-w-[150px]">{data.inputs?.condition || 'None'}</span>
            </div>
          </div>
        )}
        ...
        {data.key === 'switch_node' && (
          <div className="bg-slate-900/40 p-1.5 rounded border border-slate-800/40 font-mono text-[9px] space-y-1">
            <div className="flex items-center justify-between">
              ...
              <span className="text-purple-400 truncate max-w-[140px]">{data.inputs?.value || 'None'}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-400">Cases:</span>
              <span className="text-pink-400">{(data.inputs?.cases as string[])?.length || 0} branches</span>
            </div>
          </div>
        )}
