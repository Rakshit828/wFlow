...
export const PropertiesPanel: React.FC = () => {
  ...
  const switchCases = Array.isArray(inputs.cases)
    ? (inputs.cases as string[])
    : [];
  ...
  const handleInsertReference = (path: string, inputFieldKey: string) => {
    ...
    if (typeof currentValue === "string") {
      const isTemplateInput =
        inputFieldKey === "prompt" ||
        inputFieldKey === "body" ||
        inputFieldKey === "condition";
    ...
    return (
      <div className="mt-3 p-3 rounded-lg bg-muted/40 border border-border">
        ...
        <p className="text-xs text-muted-foreground leading-normal mb-2.5">
          Select an output path from a preceding node to inject it as a dynamic
          reference variable:
        </p>
        <div className="max-h-[140px] overflow-y-auto space-y-2 pr-1">
          {precedingNodes.map((pn) => {
            ...
            if (pn.output_model && pn.output_model.properties) {
              Object.entries(pn.output_model.properties).forEach(
                ([k, propVal]: [string, any]) => {
            ...
            return (
              <div key={pn.name} className="space-y-1">
                <span className="text-xs font-bold text-foreground block">
                  {pn.name} ({nodeRegistry[pn.key]?.name || pn.key})
                </span>
                <div className="flex flex-col gap-1 pl-1.5">
                  {outputsList.map((path) => (
                    <div
                      key={path}
                      className="flex items-center justify-between gap-1.5 bg-background/60 hover:bg-background/90 border border-border px-1.5 py-1 rounded transition-all"
                    >
                      <code className="text-xs text-amber-300 truncate tracking-wide max-w-[170px]">
                        {path}
                      </code>
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            handleInsertReference(path, targetFieldKey)
                          }
    ...
    return (
      <div className="space-y-4">
        {Object.entries(schema.properties).map(
          ([fieldKey, propSchema]: [string, any]) => {
            ...
            return (
              <div key={fieldKey} className="space-y-1">
                ...
                {propSchema.enum ? (
                  <select
                    value={
                      typeof inputs[fieldKey] === "string" ||
                      typeof inputs[fieldKey] === "number"
                        ? String(inputs[fieldKey])
                        : ""
                    }
                    onChange={(e) =>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleInputChange(fieldKey, e.target.value === "true")
                    }
                  ...
                  <div className="space-y-1">
                    <input
                      type="text"
                      value={
                        Array.isArray(inputs[fieldKey])
                          ? inputs[fieldKey].join(", ")
                          : inputStr(inputs, fieldKey)
                      }
                      onChange={(e) => {
                  ...
                  <div className="space-y-1">
                    <textarea
                      value={
                        typeof inputs[fieldKey] === "string"
                          ? inputs[fieldKey]
                          : JSON.stringify(inputs[fieldKey] ?? {}, null, 2)
                      }
                      onChange={(e) => {
    ...
    >
      ...
      <div className="p-4 border-b border-border bg-muted/30">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[11px] text-muted-foreground font-mono leading-none truncate">
            {activeNode.id}
          </span>
          <span
            className={`text-[9px] px-1.5 py-0.5 rounded-md font-semibold border shrink-0 ${getNodeTypeColor(data.type)}`}
          >
        ...
        {showSchemaInfo && data.input_model && (
          <div className="mt-2 p-2 rounded-lg bg-background/60 border border-border text-[10px] text-muted-foreground space-y-1 animate-tooltip">
            <p>
              ...
              {data.input_model.properties
                ? Object.keys(data.input_model.properties).length
                : 0}
            </p>
            {data.input_model.required && (
              <p>
                <span className="font-semibold text-foreground">Required:</span>{" "}
                {data.input_model.required.join(", ")}
              </p>
            )}
            {data.output_model && (
              <p>
                ...
                {data.output_model.properties
                  ? Object.keys(data.output_model.properties).length
                  : "dynamic"}
              </p>
            )}
          </div>
                  ...
                  <div className="space-y-3">
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-muted-foreground uppercase tracking-wide flex items-center justify-between">
                        ...
                        <span className="text-xs text-indigo-400 font-normal lowercase">
                    ...
                    {inputs.articles !== undefined && (
                      <div className="space-y-1 mt-2">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleInputChange("articles", e.target.value)
                          }
                    ...
                    {inputs.label_id !== undefined && (
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleInputChange("label_id", e.target.value)
                          }
                  ...
                  <div className="space-y-3">
                    ...
                    {inputs.spreadsheet_id !== undefined && (
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleInputChange("spreadsheet_id", e.target.value)
                          }
                    ...
                    {inputs.value_input_option !== undefined && (
                      <div className="space-y-1">
                        ...
                        <select
                          value={
                            inputStr(inputs, "value_input_option") || "RAW"
                          }
                          onChange={(e) =>
                            handleInputChange(
                              "value_input_option",
                        ...
                        >
                          ...
                          <option value="USER_ENTERED">
                            USER_ENTERED (interprets formatting/math)
                          </option>
                        </select>
                      </div>
                    )}
                  ...
                  <div className="space-y-3">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleInputChange("content_ref", e.target.value)
                        }
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleInputChange("mime_type", e.target.value)
                        }
                        className="w-full rounded-lg bg-background border border-border px-3 py-2 text-sm text-foreground focus:outline-none focus:border-indigo-500 transition-colors font-mono"
                      />
                    </div>
                    ...
                    <div className="space-y-2 mt-2 border-t border-slate-900 pt-3">
                      ...
                      <div className="space-y-2">
                        {Object.entries(ifValues).map(([vKey, vVal]) => (
                          <div
                            key={vKey}
                            className="flex gap-2 items-center bg-background/40 p-2 rounded border border-border/50"
                          >
                            <input
                              type="text"
                              value={vKey}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleInputChange("default", e.target.value)
                        }
                        className="w-full rounded-lg bg-background border border-border px-3 py-2 text-sm text-foreground focus:outline-none focus:border-indigo-500 transition-colors font-mono"
                      />
                    </div>
                        ...
                        {switchCases.map((caseName, index) => (
                          <div key={index} className="flex gap-2 items-center">
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                const cases = switchCases.filter(
                                  (_, i) => i !== index,
                                );
                                handleInputChange("cases", cases);
                              }}
              ...
              <div className="space-y-3">
                <div className="space-y-1">
                  ...
                  {nodeKey === "llm.groq" ? (
                    <select
                      value={
                        inputStr(config, "model") || "llama-3.3-70b-versatile"
                      }
                      onChange={(e) =>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="number"
                    value={
                      inputStr(config, "max_tokens") ||
                      inputStr(config, "max_output_tokens")
                    }
                    onChange={(e) => {
              ...
              <div className="p-4 rounded-lg bg-background/60 border border-border flex gap-2 items-start text-xs text-muted-foreground">
                ...
                <div>
                  ...
                  <p className="mt-1 leading-normal">
                    This action is powered by credentials resolved automatically
                    by FastAPI in real-time. No static configuration options are
                    required for this node. Use the "Inputs" tab to structure
                    its dynamically resolved parameters.
                  </p>
                </div>
              </div>
            )}
      ...
      <div className="p-3 border-t border-border bg-card/80 flex gap-2 justify-between items-center">
        <button
          onClick={() => {
            if (
              confirm(
                "Are you sure you want to delete this node and all of its edge connections?",
              )
            ) {
