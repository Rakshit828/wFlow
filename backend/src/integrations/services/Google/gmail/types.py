...
class GmailApis(str, Enum):
    ...
    GET_EMAIL_HISTORY = "gmail/v1/users/me/history"
...
class WatchGmailInput(BaseModel):
    ...
    label_ids: List[Literal["INBOX", "UNREAD", "STARRED", "SPAM"]] = Field(
        default=["INBOX"]
    )
    trigger_for: Literal["on_email_delete", "on_email_"]
...
class GetGmailHistoryInput(BaseModel):
    start_history_id: str = Field(alias="startHistoryId")
    label_id: str | None = Field(None, alias="labelId")
